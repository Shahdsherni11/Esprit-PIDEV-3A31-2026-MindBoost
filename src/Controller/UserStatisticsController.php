...
class UserStatisticsController extends AbstractController
{
    ...
    public function sendStatisticsEmail(
        ...
        if (!$user || !method_exists($user, 'getEmail')) {
            $this->addFlash('error', '❌ Utilisateur non connecté ou email introuvable.');
        ...
        $userName = method_exists($user, 'getFullName')
            ...
            : (method_exists($user, 'getUserIdentifier') ? (string) $user->getUserIdentifier() : 'Étudiant');

        // Tant que tu n'as pas encore branché l'id réel de l'utilisateur à tes scores
        // tu gardes temporairement userId = 1
        $userId = 1;
